...
from handlers.valorant.status import ValorantServerStatusHandler
...
class ValorantServerStatusHanderTest(unittest.TestCase):
    ...
    def test_filepath(self):
        ...
        self.assertEqual(result, '/valorant/randomLocale/randomRegion.status.xml')
    ...
    def test_successful_fetch(self, mock_post):
        ...
        self.assertEqual(feed.getAlternateLink(), 'https://status.riotgames.com/valorant?region=randomId&locale=randomLocale')

        item = feed.getItems()[0]
        self.assertEqual(item.getTitle(), 'Random Title')
        self.assertEqual(item.getSummary(), 'Random Text')
        self.assertEqual(item.getLink(), 'https://status.riotgames.com/valorant?region=randomId&locale=randomLocale&id=2')
