...
class ValorantServerStatusHandler(Handler):
    ...
    def get_filepath(self, server):
        return '/valorant/{locale}/{region}.status.xml'.format(region=server['region'], locale=server['locale'])

    def process_server(self, server):
        collector = ValorantServerStatusCollector(server)
...
def handle(event={}, context={}):
    ...
    ValorantServerStatusHandler().run()
    return 'ok'
