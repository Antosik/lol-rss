...
def handle(event={}, context={}):
    ...
    for locale in locales:
        for server in servers:
            ...
            filepath = '/v3/lor/{locale}/{region}.status.xml'.format(locale=locale['locale'], region=server['id'])
            fullpath = target_dir + filepath

            collector = LoRServerStatusCollector({'region': server['id'], 'id': server['id'], **locale})
